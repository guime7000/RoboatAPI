...
create_log_file(os.path.join(logsDirectoryPath,'archive.log'), logging.INFO, 'archive.log')
...
async def position(boatInfos : BoatPos):
    ...
    with open('/a_secret_place_to_store_the_pwd_file/secret_containing_pwd_file.JSON', 'r') as pwdf :
        pwd = json.load(pwdf)
    ...
    if pwd[boatInfos.inName] != boatInfos.inPwd :
        ...
        archivePositionFile = os.path.join(archiveDirectoryPath,boatInfos.inName + '_Arch.JSON')
